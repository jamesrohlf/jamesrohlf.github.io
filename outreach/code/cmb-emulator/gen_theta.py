...
def spectrum(theta):
    ...
    try:
        ...
        if not np.all(np.isfinite(d)) or np.any(d <= 0):
            return None
        return d.astype(np.float64), float(res.Params.H0)
    except Exception:
        return None
...
def main(n, seed, out):
    ...
    X  = X[ok]
    Y  = np.array([out_list[i][0] for i in ok])
    H0 = np.array([out_list[i][1] for i in ok])
    ell = np.arange(LMIN, LMAX+1)
    np.savez_compressed(out, X=X, Y=Y, H0=H0, ell=ell,
